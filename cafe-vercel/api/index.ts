...
// Supabase client using the service_role key — server-side only, bypasses RLS.
// NEVER expose SUPABASE_SERVICE_ROLE_KEY to the frontend.
const MENU_IMAGES_BUCKET = process.env["SUPABASE_STORAGE_BUCKET"] || "menu-images";
 ...
 * Uploads image to the Supabase "menu-images" bucket (public) and returns its public URL.
 */
app.post(
  "/api/storage/upload",
  upload.single("file"),
  async (req: Request & { file?: Express.Multer.File }, res: Response): Promise<void> => {
    ...
    if (!supabaseAdmin) {
      res.status(500).json({
        error: "Supabase storage not configured. Set SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY.",
      });
      return;
    }
